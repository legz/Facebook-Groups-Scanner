...
scan () {
	...
	cat group.html.tmp | grep -oP '(?<=1">).{0,500}application .{0,500}(?=</span></span>)' > output.tmp
	cat output.tmp | sed "s/<\/a>&nbsp;<span>/ : /g" > outputClean1.tmp
	cat outputClean1.tmp | sed "s/<a href.\+>\(.\+\)<\/a>/[\1]/g" > outputClean2.tmp
	cat outputClean2.tmp | perl -MHTML::Entities -pe 'decode_entities($_);' > output.tmp
